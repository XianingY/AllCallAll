...
test("opens a conversation and renders messages with business context", async ({ page }) => {
  ...
  await page.route("**/api/v1/conversations/12/messages", (route) => route.fulfill({ json: { messages: [{ id: 1, organization_id: 7, conversation_id: 12, sender_id: 2, sender_email: "pm@example.com", sender_display_name: "产品经理", type: "text", body: "请确认行动项", created_at: "2026-06-27T10:00:00Z" }] } }));
  ...
  await expect(page.getByText("请确认行动项").last()).toBeVisible();
});
