...
function RemoteStream({ stream, blocked, onBlocked }: { stream: MediaStream; blocked: boolean; onBlocked: () => void }) {
...
export function MeetingRoomPage() {
  ...
  const localVideo = useRef<HTMLVideoElement>(null); const [autoplayBlocked, setAutoplayBlocked] = useState(false);
  ...
  return <div className="meeting-room"><header><div><h1>{room.data.room.title}</h1><span><span className={`connection-dot ${meeting.state === "connected" ? "online" : ""}`} />{meeting.state}</span></div><div className="meeting-header-actions"><span><Users size={15} />{room.data.participant_count}/6</span>{room.data.latest_recording_id && <Link to={`/recordings/${room.data.latest_recording_id}`}><FileAudio size={16} />转写</Link>}</div></header>
    <main className="meeting-stage"><div className="remote-media">{remoteStreams.length === 0 ? <div className="meeting-wait"><Users size={34} /><strong>等待其他参会人</strong><span>已加入 {room.data.participant_count} 人</span></div> : remoteStreams.map((stream, index) => <RemoteStream key={stream.id || index} stream={stream} blocked={autoplayBlocked} onBlocked={handleAutoplayBlocked} />)}{autoplayBlocked && <button className="autoplay-button" onClick={() => { setAutoplayBlocked(false); }}><Volume2 size={17} />播放远端音频</button>}</div><div className="local-media"><video ref={localVideo} autoPlay muted playsInline />{!meeting.video && <VideoOff size={22} />}</div>{meeting.error && <div className="meeting-error">{meeting.error}</div>}</main>
