    ...
    <>
      <header>
        ...
        <div className="result-badges">
          {runtimeLabel && <span className="transcription-badge">{runtimeLabel}</span>}
          <span className={`transcription-badge status-${status}`}>{status || "idle"}</span>
        </div>
      </header>
      <section>
        <h3>摘要</h3>
    ...
    <div>
      <div className="approval-toolbar">
        ...
        <div className="segmented">
          <button className={pendingOnly ? "active" : ""} onClick={() => setPendingOnly(true)}>
            待处理
          </button>
          <button className={!pendingOnly ? "active" : ""} onClick={() => setPendingOnly(false)}>
            全部
          </button>
        </div>
      </div>
      <div className="approval-list">
        {visible.map((item) => (
          <article className="panel" key={item.id}>
            ...
            {item.status === "pending" && (
              <footer>
                <button className="button-secondary" onClick={() => decide(item.id, "reject")}>
                  <X size={16} />拒绝
                </button>
                <button className="button-primary" onClick={() => decide(item.id, "approve")}>
                  <Check size={16} />批准
                </button>
              </footer>
            )}
...
export function RerankPanel({
  ...
  const reranked = agenticRAG?.rerankedHits.length ? agenticRAG.rerankedHits : ranked;
  if (!reranked.length && !agenticRAG) return <div className="pane-empty">暂无可展示的 RAG/Rerank 引用</div>;
  return (
  ...
  return (
    <section>
      ...
      <article className="panel citation-card">
        <header>
          <strong>{agenticRAG.harness?.name || "bounded retrieval loop"}</strong>
          <span>{agenticRAG.sufficiency?.sufficient === false ? "insufficient" : "grounded"}</span>
        </header>
        <footer className="citation-meta">
          {agenticRAG.route?.route && <span>{agenticRAG.route.route}</span>}
          {agenticRAG.route?.retrieval_strategy && <span>{agenticRAG.route.retrieval_strategy}</span>}
          <span>max {agenticRAG.plan?.max_steps ?? agenticRAG.budget?.max_steps ?? 0}</span>
          <span>
            confidence{" "}
            {(agenticRAG.evidence?.confidence ?? agenticRAG.sufficiency?.confidence ?? 0).toFixed(2)}
          </span>
          {agenticRAG.evidence?.source_types?.map((source) => (
            <span key={source}>{sourceLabel(source)}</span>
          ))}
        </footer>
        {agenticRAG.harness?.prompt_version && <p>prompt: {agenticRAG.harness.prompt_version}</p>}
        {agenticRAG.stopReason && <p>stop: {agenticRAG.stopReason}</p>}
        {agenticRAG.sufficiency?.reason && <p>{agenticRAG.sufficiency.reason}</p>}
      ...
      {agenticRAG.loopTraces.length ? (
        <div className="approval-list">
          {agenticRAG.loopTraces.map((loop) => (
            <article className="panel citation-card" key={`${loop.role}-${loop.stop_reason}`}>
              <header>
              ...
              <footer className="citation-meta">
                <span>
                  {loop.budget?.used_steps ?? 0}/{loop.budget?.max_steps ?? loop.spec?.max_steps ?? 0} steps
                </span>
      ...
      <div className="approval-list">
        {agenticRAG.attempts.map((attempt, index) => (
          <article className="panel citation-card" key={`${attempt.step ?? index}-${attempt.tool_name ?? "tool"}`}>
            <header>
            ...
            {attempt.expanded_terms?.length ? <pre>{attempt.expanded_terms.join(", ")}</pre> : null}
...
function ChunkList({ title, chunks }: { title: string; chunks: AgenticRAGChunk[] }) {
  return (
    <section>
      <h2>
        {title}
      ...
      <div className="approval-list">
        {chunks.map((chunk, index) => (
          <RerankChunkCard chunk={chunk} index={index} key={`${title}-${chunk.source_type}-${chunk.source_id}-${index}`} />
        ))}
      </div>
    </section>
  );
...
function RerankChunkCard({ chunk, index }: { chunk: AgenticRAGChunk; index: number }) {
  return (
    <article className="panel citation-card">
      <header>
        <strong>{chunk.source_title || chunk.title || sourceLabel(chunk.source_type || "source")}</strong>
      ...
      <footer className="citation-meta">
        ...
        {chunk.rerank_score ? <span>rerank {chunk.rerank_score.toFixed(2)}</span> : null}
      </footer>
