...
func resolveCommand(command string, environment map[string]string) (string, error) {
	...
	// 含路径分隔符的命令不再原样放行：规范化后必须是绝对路径、不含 ".." 穿透，
	// 且必须是带执行权限的常规文件，避免执行目录/设备文件或经 ".." 越权访问，
	// 同时仍允许沙箱运行时合法请求的绝对路径二进制（如 python 解释器）。
	// Commands carrying a path separator are no longer passed through as-is: after
	// cleaning they must be an absolute path with no ".." traversal and a regular
	// file with an execute bit, which blocks directories/devices and path traversal
	// while still permitting legitimate absolute binaries (e.g. the python runtime).
	if containsPathSeparator(command) {
		if !filepath.IsAbs(cleaned) || strings.Contains(cleaned, "..") {
			return "", fmt.Errorf("command %q is not an allowed absolute path: %w", command, ErrSessionRejected)
		}
		info, err := os.Stat(cleaned)
		if err != nil || !info.Mode().IsRegular() || info.Mode().Perm()&0o111 == 0 {
			return "", fmt.Errorf("command %q is not an executable regular file: %w", command, ErrSessionRejected)
		}
		return cleaned, nil
	}
