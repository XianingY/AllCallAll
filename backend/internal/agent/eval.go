...
func RunPlannerEval(ctx context.Context, planner Planner, cases []EvalCase) (EvalReport, error) {
	if planner == nil {
		return EvalReport{}, ErrPlannerUnavailable
	}
	report := EvalReport{
...
func (item EvalCase) toPlannerInput(index int) PlannerInput {
...
func validateEvalOutput(item EvalCase, output PlannerOutput) []string {
	var errs []string
