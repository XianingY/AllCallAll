...
func executeReActTaskEval(ctx context.Context, svc *Service, orgID, userID, conversationID uint64, item AgentTaskEvalCase) (AgentTaskEvalResult, error) {
	queued, err := svc.RunConversationAssistant(ctx, orgID, userID, RunInput{
	...
	eval := AgentTaskEvalResult{
		...
		SummaryPreview:  compactSnippet(result.Run.Summary, 160),
		NextStepPreview: compactSnippet(result.Run.NextStep, 120),
	}
...
func executeWorkflowTaskEval(ctx context.Context, svc *Service, orgID, userID, conversationID uint64, item AgentTaskEvalCase) (AgentTaskEvalResult, error) {
	created, err := svc.StartWorkflowAgent(ctx, orgID, userID, WorkflowInput{
		ConversationID: conversationID,
		Goal:           item.Prompt,
		Preset:         item.Preset,
	})
...
func taskEvalCitationPresent(citations []Citation, requiredTypes []string) bool {
...
func taskEvalMeetingGrounded(citations []Citation, item AgentTaskEvalCase) bool {
	if len(item.SeedMeetingTranscripts) == 0 && !containsExact(item.RequiredCitationSourceTypes, contextChunkSourceMeetingTranscript) {
		return true
	}
	return workflowEvalCitationTypePresent(citations, contextChunkSourceMeetingTranscript)
}
...
func workflowUsedTools(result *WorkflowResult) []string {
