...
func parseFlags() benchConfig {
	...
	flag.StringVar(&cfg.Provider, "provider", models.AgentRunSourceRules, "agent provider: rules, mock_llm, or openai_compatible")
...
func run(ctx context.Context, cfg benchConfig, writer io.Writer) error {
	...
	if err := migrateBenchTables(db); err != nil {
	...
	registerBenchHandlers(processor, agentSvc, &executeDurations)

	queueDurations := make([]time.Duration, 0, cfg.Conversations)
	if err := seedBenchRuns(ctx, db, agentSvc, cfg.Conversations, &queueDurations); err != nil {
		return err
	}
	...
	output, err := buildBenchOutput(ctx, db, started, cfg, processed, queueDurations, executeDurations, counters.Snapshot(), dbPath)
	if err != nil {
		return err
	}
...
func migrateBenchTables(db *gorm.DB) error {
...
func registerBenchHandlers(processor *events.Processor, agentSvc *agent.Service, executeDurations *[]time.Duration) {
...
func seedBenchRuns(ctx context.Context, db *gorm.DB, agentSvc *agent.Service, count int, queueDurations *[]time.Duration) error {
	for i := 0; i < count; i++ {
		actorID := uint64(1000 + i)
		contactID := uint64(2000 + i)
		conversation, err := seedBenchConversation(ctx, db, uint64(1), actorID, contactID, i)
		if err != nil {
			return err
		}
		started := time.Now()
...
func seedBenchConversation(ctx context.Context, db *gorm.DB, organizationID, actorID, contactID uint64, index int) (*models.Conversation, error) {
	assigneeID := actorID
...
func buildBenchOutput(ctx context.Context, db *gorm.DB, started time.Time, cfg benchConfig, processed int, queueDurations []time.Duration, executeDurations []time.Duration, counters map[string]int64, dbPath string) (*benchOutput, error) {
	output := &benchOutput{
