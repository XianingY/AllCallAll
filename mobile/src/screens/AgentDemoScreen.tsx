  ...
  const renderSidebar = () => (
    <View style={isWide ? styles.sidebar : styles.panel}>
      ...
      <Text style={styles.heading}>
        {knowledgeOnly ? "Knowledge Center" : "Agent Lab"}
      </Text>
      {!knowledgeOnly ? (
      ...
      <View style={styles.actionRow}>
        ...
        />
        <PrimaryButton
          title={knowledgeOnly ? "Open Lab" : "Demo"}
          onPress={
            knowledgeOnly
              ? () => navigation.navigate("AgentDemo")
    ...
    <ScrollView style={styles.main} contentContainerStyle={styles.mainContent}>
      ...
      <View style={styles.grid}>
        <View style={[styles.panel, styles.formPanel]}>
          ...
          <PrimaryButton
            title="Add Text"
            onPress={handleCreateManualSource}
        ...
        <View style={[styles.panel, styles.listPanel]}>
          ...
          {sources.map((source) => (
            <Pressable
              ...
              onPress={() => void selectSource(source.id)}
            >
              ...
              <View style={styles.inlineActions}>
                <Pressable
                  ...
                  onPress={() =>
                    void reingestKnowledgeSource(token ?? "", source.id).then(
                      refreshKnowledge,
                    )
                  }
                >
                  ...
                  <View key={source.id} style={styles.inlineRow}>
                    ...
                    {group.canonical_source_id !== source.id ? (
                      <Pressable
                        style={styles.inlineButton}
                        onPress={() =>
                          void handleSetCanonical(group.id, source.id)
                        }
                      >
              ...
              {item.status === "pending" ? (
                <View style={styles.inlineActions}>
                  <Pressable
                    ...
                    onPress={() =>
                      void handleDuplicateDecision(item.id, "confirm")
                    }
                  >
                  ...
                  <Pressable
                    ...
                    onPress={() =>
                      void handleDuplicateDecision(item.id, "reject")
                    }
                  >
        ...
        {deadLetters.map((item) => (
          <View key={item.id} style={styles.rowItem}>
            ...
            <Pressable
              style={styles.inlineButton}
              onPress={() =>
                void retryKnowledgeDeadLetter(token ?? "", item.id).then(
                  refreshKnowledge,
                )
              }
            >
    ...
    <ScrollView style={styles.main} contentContainerStyle={styles.mainContent}>
      <View style={styles.panel}>
        <View style={styles.panelHeader}>
          ...
          <PrimaryButton
            title="Process"
            onPress={handleProcessWorkflow}
            disabled={busy || !activeWorkflow}
          ...
          <View key={approval.id} style={styles.approvalBox}>
            ...
            {approval.status === "pending" ? (
              <View style={styles.inlineActions}>
                <Pressable
                  style={styles.approveButton}
                  onPress={() => void handleApproval(approval, "approve")}
                >
                  <Text style={styles.approveButtonText}>Approve</Text>
                </Pressable>
                <Pressable
                  style={styles.rejectButton}
                  onPress={() => void handleApproval(approval, "reject")}
                >
                  <Text style={styles.rejectButtonText}>Reject</Text>
                </Pressable>
              </View>
      ...
      <View style={styles.panel}>
        ...
        {workflows.map((item) => (
          <Pressable
            ...
            style={styles.rowItem}
            onPress={() => setActiveWorkflow(item)}
          >
            <View style={styles.rowTop}>
  ...
  return (
    <View style={styles.container}>
      <View style={isWide ? styles.desktopLayout : styles.mobileLayout}>
        ...
        {!knowledgeOnly && activeTab === "run" ? renderRunTab() : null}
        {!knowledgeOnly && activeTab === "graph" ? renderGraphTab() : null}
        {!knowledgeOnly && activeTab === "approvals"
          ? renderApprovalsTab()
          : null}
        {!knowledgeOnly && activeTab === "eval" ? renderEvalTab() : null}
      </View>
    </View>
  );
};
