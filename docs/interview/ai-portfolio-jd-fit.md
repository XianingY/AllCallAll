...
| Intelligent Agent design | Go Agent runtime plus Python FastAPI + LangGraph runtime for `meeting_brief`, `risk_review`, `follow_up_planner`, and `context_qa`. | Go remains source of truth for auth, data, approvals, audit, and writes. |
...
| Hybrid retrieval and rerank | Explicit Go `Reranker` abstraction with `rules` and `cross_encoder_compatible` providers; eval reports compare baseline vs reranked Top-K. | `rules` is used for reproducible evidence; cross-encoder HTTP provider is configurable but not required for CI. |
...
> I did not move all business logic into Python. Go owns product data, permissions, approval, audit, and writes. Python is the AI orchestration layer where LangGraph, LangChain prompt templates, rerank/grounding trace, and eval iterate faster. This gives the project a clear AI full-stack boundary instead of a fragile all-in-one Agent service.
