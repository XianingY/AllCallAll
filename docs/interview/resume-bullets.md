...
- Extracted a Python FastAPI + LangGraph Agent Runtime for selected workflow presets: Go keeps auth, data ownership, read-tool authorization, tool policy, approvals, audit, and final writes, while Python handles workflow registry, DAG orchestration, bounded ReAct role loops, provider adapters, trace events, citations, and write-tool proposals.
...
- Added a Python-side Agent Runtime eval runner covering task success, citation grounding, tool intent match, approval safety, prompt schema presence, grounding-check trace, Agentic RAG refinement, citation coverage, iteration compliance, and unsupported-claim guarding across deterministic LangGraph task fixtures.
