...
> Fact calibration (July 19, 2026): the current interview acceptance path is
> `make interview-smoke && make interview-chaos`. Numbers below dated June 2026 are retained as
> historical local snapshots only; they are not regenerated by the Compose acceptance command and
> must not be presented as current capacity or SLA. Current deterministic artifacts are regenerated by
> `make agent-demo-report`, `make resume-eval`, `make python-agent-eval`, and `make python-rag-eval`.
