...
This directory is a **stale mirror** of the Agent Runtime code that now lives in
...
- The standalone repo is the authoritative source:
  `allcallall-agent-runtime/services/agent-runtime`.
...
- GitHub Actions CI installs and builds this in-tree copy today
  (`agent-runtime/Dockerfile`, `agent-runtime/pyproject.toml`), so the directory
  is intentionally retained until CI is retargeted to the standalone repo.
